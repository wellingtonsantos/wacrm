      ...
      {tagNames.map((name) => {
        ...
        return (
          <span
            ...
            title={isKnown ? name : `${name} (será criada ao importar)`}
          >
            <span
...
export function ImportModal({
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (rows.length === 0) {
      toast.error(
        'Nenhuma linha válida encontrada. Certifique-se de que o CSV tem um cabeçalho de coluna chamado "phone".'
      );
      setParsedRows([]);
      setHasTagsColumn(false);
      setHasCompanyColumn(false);
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (tagsAssigned > 0) {
        toast.success(
          `${tagsAssigned} atribuição${tagsAssigned !== 1 ? 'ões' : 'ão'} de tag aplicada${tagsAssigned !== 1 ? 's' : ''}`
        );
      }
            ...
            <div className="space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-[11px] font-semibold tracking-[0.14em] text-muted-foreground uppercase">
                  Pré-visualização · primeira{preview.length !== 1 ? 's' : ''} {preview.length}
                </p>
              ...
              {parsedRows.length > PREVIEW_LIMIT && (
                <p className="text-center text-[11px] text-muted-foreground">
                  + {parsedRows.length - PREVIEW_LIMIT} outra{parsedRows.length - PREVIEW_LIMIT !== 1 ? 's' : ''} linha{parsedRows.length - PREVIEW_LIMIT !== 1 ? 's' : ''} não exibida{parsedRows.length - PREVIEW_LIMIT !== 1 ? 's' : ''}
                </p>
              )}
            </div>
          )}
