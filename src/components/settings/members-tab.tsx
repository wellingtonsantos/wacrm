...
const EDITABLE_ROLES: { value: AccountRole; label: string; hint: string }[] = [
  { value: 'admin', label: 'Administrador', hint: 'Gerenciar membros e tudo mais' },
  { value: 'agent', label: 'Agente', hint: 'Usar recursos; sem configurações' },
  { value: 'viewer', label: 'Visualizador', hint: 'Apenas visualização em todo o app' },
];
...
export function MembersTab() {
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      ...
      toast.success(`Função de ${member.full_name || 'membro'} atualizada para ${ROLE_META[nextRole].label}`);
    } catch (err) {
        ...
        <div>
          ...
          {invitations.length > 0 ? (
            <p className="mb-3 text-xs text-muted-foreground">
              A URL do convite em texto simples é exibida apenas uma vez no momento da criação
        ...
        <DialogContent className="bg-popover border-border sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              ...
              da conta? Ele(a) será desconectado(a) desta conta e receberá uma nova conta pessoal no próximo acesso. O login dele(a) não será excluído.
            </DialogDescription>
          </DialogHeader>
